...
function App() {
  const [buttonBackgroundColor, setButtonBackgroundColor] =
    useState("MediumVioletRed");
  const [checked, setChecked] = useState(false);

  const newButtonBackgroundColor =
    buttonBackgroundColor === "MediumVioletRed"
      ? "MidnightBlue"
      : "MediumVioletRed";
  const buttonText = `Change to ${replaceCamelWithSpaces(
  ...
  return (
    <div>
      <button
        style={{ backgroundColor: checked ? "gray" : buttonBackgroundColor }}
        onClick={handleClick}
        disabled={checked}
      >
        {buttonText}
